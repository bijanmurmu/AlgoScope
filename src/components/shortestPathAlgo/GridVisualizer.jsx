...
const getCellClassName = (node) => {
  return `w-7 h-7 border border-[var(--theme-border)] flex items-center justify-center text-[11px] text-[var(--theme-text-strong)] ${
    node.isStart
      ? 'bg-green-500'
      : node.isEnd
        ? 'bg-red-500'
...
const GridVisualizer = ({ algorithm, runKey, speed }) => {
  ...
  return (
    <div className="w-full bg-[var(--theme-surface-muted)] border border-[var(--theme-border)] p-4 rounded-xl">
      <div className="flex flex-wrap gap-3 mb-5">
        {DRAW_MODES.map((mode) => (
          <button
            ...
            className={`px-4 py-2 rounded-lg text-[var(--theme-text-strong)] font-semibold text-sm ${drawMode === mode ? 'bg-cyan-600' : 'bg-[var(--theme-surface-strong)]'}`}
          >
        ...
        <button
          ...
          disabled={running}
          className="px-4 py-2 bg-[var(--theme-surface-strong)] rounded-lg text-[var(--theme-text-strong)] font-semibold text-sm"
        >
          Generate Maze
        </button>
        <button
      ...
      <div className="flex gap-6">
        <div className="w-64 space-y-4">
          <div className="bg-[var(--theme-surface)] border border-[var(--theme-border)] p-4 rounded-lg text-[var(--theme-text-strong)] text-sm">
            <h3 className="font-bold mb-2">HOW TO USE</h3>
            <ol className="text-[var(--theme-text-muted)] text-xs list-decimal pl-4 space-y-1">
        ...
        <div
          className="inline-block border border-[var(--theme-border-strong)] overflow-hidden rounded-lg"
      ...
      <div className="flex gap-4 mt-5">
        <div className="bg-[var(--theme-surface)] border border-[var(--theme-border)] px-4 py-2 rounded-lg text-[var(--theme-text)] text-sm">
      ...
      <div className="flex flex-wrap gap-4 mt-5 text-xs text-[var(--theme-text-muted)]">
        {LEGEND_ITEMS.map((i) => (
